...
from app.schemas import CommandeCreate, CommandeResponse
...
def list_commandes(db: Session = Depends(get_db)):
    ...
    return db.query(HistoriqueCommande).all()

@router.get("/actionneur/{actionneur_id}", response_model=list[CommandeResponse])
def get_commandes_actionneur(actionneur_id: int, db: Session = Depends(get_db)):
    ...
    return db.query(HistoriqueCommande).filter(HistoriqueCommande.actionneur_id == actionneur_id).all()
