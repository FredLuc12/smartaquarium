...
const String NGROK_URL = "http://635f291a4c29c99c-91-167-32-196.serveousercontent.com";  // HTTP !
const String API_URL_MESURE = NGROK_URL + "/api/mesures/";
const String API_URL_COMMAND = NGROK_URL + "/api/commandes/";
const String API_URL_ACTIONNEURS = NGROK_URL + "/api/actionneurs/";
const String API_URL_CAPTEURS = NGROK_URL + "/api/capteurs/";
...
const int RELAY_HEAT = 22;
const int RELAY_PUMP = 23;
const int RELAY_LUM = 19;
...
void Setup_Hour(){
  ...
  Serial.println("✅ RTC DS1307 detecte !");

  rtc.adjust(DateTime(2026, 1, 28, 18, 48, 0));  // Année, Mois, Jour, Heure, Minute, Seconde
...
void Trigger_Call_API(int RELAY, int &last_value, int actionneur_id, int capteur_id, float mesure){
  ...
  if (current_state != last_value){
    Serial.println("🔄 Changement d'état actionneur " + String(actionneur_id));
    last_value = current_state;
    
    // Envoyer SEULEMENT la commande (pas la mesure ici)
    String payload_commande = "{"
      "\"actionneur_id\":" + String(actionneur_id) + ","
      "\"commande\":\"" + String(current_state == HIGH ? "ON" : "OFF") + "\","
      "\"source\":\"automatique\"" +
      "}";
...
void Send_Mesure(int capteur_id, float valeur) {
  String payload = "{"
    "\"capteur_id\":" + String(capteur_id) + ","
    "\"valeur\":" + String(valeur) +
    "}";
  
  Serial.println("📡 Mesure capteur " + String(capteur_id));
  Send_JSON(payload, API_URL_MESURE);
}

unsigned long lastMesureTime = 0;
const unsigned long MESURE_INTERVAL = 60000; // 60 secondes
...
void loop() {
  ...
  float light_result = (float)Photoresistor();
  ...
  // ✅ Contrôler les relays (avec envoi commandes si changement)
  Relay_Pump(distance_result);
  Relay_Heat(temperature_result);
  //Relay_Lum(light_result);

  // ✅ NOUVEAU : Envoyer les mesures toutes les 60 secondes
  unsigned long currentTime = millis();
  if (currentTime - lastMesureTime >= MESURE_INTERVAL) {
    lastMesureTime = currentTime;
    
    Serial.println("\n📊 Envoi périodique des mesures...");
    
    // Envoyer toutes les mesures
    Send_Mesure(1, temperature_result);
    delay(500);
    Send_Mesure(2, distance_result);
    delay(500);
    Send_Mesure(4, pH_result);
    delay(500);
    Send_Mesure(3, light_result);
  }
